...
namespace LiteDB
{
    ...
    internal class Reflection
    {
            ...
            lock (_cacheCtor)
            {
                try
                {
                    ...
                    if (typeInfo.IsClass)
                    {
                        ...
                        c = () => { return type.GetConstructor(Type.EmptyTypes).Invoke(null); };
                        _cacheCtor.Add(type, c);
                    }
                    else if (typeInfo.IsInterface) // some know interfaces
                    {
                        ...
                        if (typeInfo.IsGenericType && type.GetGenericTypeDefinition() == typeof (ICollection<>))
                        {
                            return CreateInstance(GetGenericListOfType(UnderlyingTypeOf(type)));
                        }
                    ...
                    else // structs
                    {
                        /*var dynMethod = new DynamicMethod("_", typeof(object), null);
                            ...
                            c = (CreateObject)dynMethod.CreateDelegate(typeof(CreateObject));
                            _cacheCtor.Add(type, c);*/
                        c = () => { return Activator.CreateInstance(type); };
                        _cacheCtor.Add(type, c);
                    }
        ...
        private static GenericGetter CreateGetMethod(Type type, PropertyInfo propertyInfo, bool nonPublic)
        {
            ...
            return obj => { return getMethod.Invoke(obj, null); };
        }
...
/*
            ...
            return (target, value) => { return setMethod.Invoke(target, new[] {value}); };
        }
